...
This command will run the plugin compiler and create a Go plugin called `SharedQuotaPlugin.so` 
which can be found in `tyk/middleware/SharedQuotaPlugin.so` after it successfully builds. This `.so` file can be loaded 
