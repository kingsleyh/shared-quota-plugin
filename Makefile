...
go-build: go/src/go.mod
	...
	docker-compose run --rm tyk-plugin-compiler SharedQuotaPlugin.so _$$(date +%s)
	mv -f ./go/src/SharedQuotaPlugin*.so ./tyk/middleware/
...
go-clean:
	...
	-rm -rf ./go/src/go.mod
	-rm -rf ./go/src/go.sum
	-rm -f ./tyk/middleware/SharedQuotaPlugin*.so
	-rm -f ./tyk/bundle/SharedQuotaPlugin*.so
